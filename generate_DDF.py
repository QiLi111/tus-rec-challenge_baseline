...
#     Evaluation_test.generate_label_values(scan_index)
...
for scan_index in range(0,len(list(Evaluation_test.Prediction_Global_AllPts_keys_only.keys()))):
    # predictions
    Evaluation_test.calculate_GT_DDF(scan_index)
    Evaluation_test.generate_pred_values(scan_index)
    Evaluation_test.scan_plot(scan_index)
    print('%s is finished.'%Evaluation_test.Prediction_Global_AllPts_keys_only.keys()[scan_index])
